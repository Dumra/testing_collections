...
pub fn calculate_mean(vector: &[i32]) -> i32 {
    let mut sum = 0i32;
...
pub fn calculate_median(vector: &mut Vec<i32>) -> i32 {
    ...
    if count % 2 != 0 {
        ...
        get_value_from_vector(index as usize, &vector)
    } else {
        let index = (count / 2) - 1;
        ...
        (first_middle_value + second_middle_value) / 2
    }
}

pub fn calculate_mode(vector: &[i32]) -> (i32, u32) {
...
fn get_value_from_vector(index: usize, vector: &[i32]) -> i32 {
